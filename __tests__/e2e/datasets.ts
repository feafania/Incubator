...
import {Collection, InsertManyResult, OptionalId, OptionalUnlessRequiredId} from "mongodb";
...
// 	        insertedIds — объект с ключами индексов вставленных элементов и их _id.
    ...
    try {
        ...
        console.error('An error occurred while initializing the collection:', error);
        return null;
    }
}

export async function setMongoDB1<T extends Document>(
...
// export async function setBlogsMongoDB(
//     collection: Collection<BlogDBType>,
//     dataset: OptionalUnlessRequiredId<BlogDBType>[]//_id не обязателен, если оно не указано.
// ): Promise<InsertManyResult<BlogDBType> | null> {
//
//     try {
//         if ((await collection.countDocuments()) > 0) {
//             await collection.drop();
//         }
//         if (dataset.length > 0)  {
//             return await collection.insertMany(dataset);
//         }
//         return null;
//     } catch (error) {
//         console.error('An error occurred while initializing the collection:', error);
