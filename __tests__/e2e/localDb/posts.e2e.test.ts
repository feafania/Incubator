...
describe("tests for /posts", () => {
  ...
  it("should get not empty posts array", async () => {
    ...
    // expect(res.body[0]).toEqual(postsService.mapToOutput(datasetPostValid[0]));
    expect(res.body).toHaveProperty("items");
    expect(Array.isArray(res.body.items)).toBe(true);
    expect(res.body.items.length).toBe(datasetPostValid.length);

    const sortedExpected = await Promise.all([...datasetPostValid]
  ...
  it("should delete all posts", async () => {
    ...
    const res = await agent
      ...
      .expect(HTTP_STATUSES.NO_CONTENT_204);
    console.log("after delete", db.posts, res.status);
  });
});
