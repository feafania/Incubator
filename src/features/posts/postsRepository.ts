...
export const postsRepository = {
    ...
    async findPosts(title: string | null | undefined): Promise<ViewPostModel[]> {
        if (title) {
            return db.posts.filter(post => post.title.indexOf(title) > -1).map(postsRepository.mapToOutput);
        }
        else
        {
    ...
    async create (input: CreatePostInputModel): Promise<postReturnType> {
        ...
        return {post: this.mapToOutput(newPost)}
    },
    async update (index:number, input: UpdatePostInputModel): Promise<postReturnType> {
        ...
        return { post: this.mapToOutput(updatedPost) };
    },
    mapToOutput(model: PostDBType): ViewPostModel {
        return {
            ...
            blogName: db.blogs.filter((blog) => blog.id === model.blogId)[0]?.name || '',
        }
    },
